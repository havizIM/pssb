<div class="content-header row">
  <div class="content-header-left col-md-6 col-12 mb-2">
    <h3 class="content-header-title">Laporan Pendaftaran</h3>
      ...
      <div class="col-12">
        <div class="card">
          <div class="card-header">
            <h4 class="card-title">Filter Laporan Pendaftaran</h4>
          </div>
                    ...
                    <div class="table-responsive col-sm-12">
                      <table class="table">
                        ...
                        <tbody>`
                         ...
                         var total_tolak = 0;
                         $.each(data, function(k, v){
                           if(v.status_pendaftaran === 'Terima'){
                            total_terima++;
                           }

                           if(v.status_pendaftaran === 'Tolak'){
                            total_tolak++;
                           }
                           ...
                           html += `
                             <tr>
                                <td>${no++}</td>
                                <td>${v.id_pendaftar}</td>
                                <td>${v.nama_lengkap}</td>
                                <td>${v.jenis_kelamin}</td>
                                <td>${v.status_pendaftaran}</td>
                                <td>${v.tgl_register}</td>
                              </tr>
                           `
                         })
                        ...
                        <table class="table">
                          <tbody>
                            <tr>
                              ...
                              <td class="text-right">${total_terima}</td>
                            </tr>
                            <tr>
                              <td>Total Ditolak</td>
                              <td class="text-right">${total_tolak}</td>
                            </tr>
                            <tr class="bg-grey bg-lighten-4">
                              <td class="text-bold-800">Total Pendaftar</td>
                              <td class="text-bold-800 text-right">${data.length}</td>
                            </tr>
                          </tbody>
                        </table>
                      </div>
    ...
    var submitLaporan = () => {
      $('#form_laporan').on('submit', function(e){
        ...
        if(kd_ta === ''){
          ...
          $.ajax({
            url: `<?= base_url('api/pendaftaran/show/'); ?>${auth.token}?kd_ta=${kd_ta}`,
            type: 'GET',
            dataType: 'JSON',
            beforeSend: function(){
